...
const Text: FC<{ children: string | Array<ReactNode> }> = ({ children }) => (
...
const BadgeWithTooltip: FC<{ badge: Badge; img: string; title?: string }> = ({
  badge,
  img,
  title,
}) => {
...
const Message: FC<Props> = ({ chatMessage, emotes, badges }) => {
  ...
  const renderBadge = ({ badge, value, title }: RenderBadgeProps) => {
    ...
    return (
      <BadgeWithTooltip
        img={selectedBadge.image_url_1x}
        badge={badge}
        title={title}
      />
    );
  ...
  const parseMessage = (): Array<ReactNode | Array<ReactNode>> => {
    const emotesIndexes = message
      .split(" ")
      .map((word, index) => {
        const emoteFromEmotesList = toPairs(testEmotes || {}).find(
          ([badge, positions]) =>
            positions.some((position) => {
              const [start, end] = position.split("-");
              const wordFromPosition = message.substring(
                Number(start),
                Number(end) + 1
              );
    ...
    return emotesIndexes.map((emoteIndex, index) => {
      ...
      const previousEmoteIndex = has("badge", emotesIndexes[index - 1])
        ? (emotesIndexes[index - 1] as EmoteInMessage).index
        : (emotesIndexes[index - 1] as number);

      if (has("badge", emoteIndex)) {
        const hasTextAfterLastElement =
          equals(index, dec(emotesIndexes.length)) &&
          !equals(emoteIndex.index, dec(message.split(" ").length));

        const hasWordBetweenEmote = gt(
          emoteIndex.index - previousEmoteIndex,
          1
        );
            ...
            {pipe(
              slice(
                isFirstElement
                  ? 0
                  : hasWordBetweenEmote
                  ? previousEmoteIndex + 1
                  : emoteIndex.index,
                emoteIndex.index
              ) as (list: Array<string>) => Array<string>,
              renderWords
            )(message.split(" "))}
          </Text>,
          <EmoteWithTooltip
            emote={{ name: emoteIndex.word, url: emoteIndex.url }}
          />,
          hasTextAfterLastElement ? (
            <Text>
              {pipe(
                slice(
                  isFirstElement ? 0 : emoteIndex.index + 1,
                  message.length - 1
                ) as (list: Array<string>) => Array<string>,
                renderWords
              )(message.split(" "))}
      ...
      const hasTextAfterLastElement =
        equals(index, dec(emotesIndexes.length)) &&
        !equals(emoteIndex, dec(message.split(" ").length));
  ...
  const usernameColor =
    getContrastRatio(color || "#fff", theme.palette.background.default) < 4
      ? lighten(color || "#fff", 0.4)
      : color;

  const messageBadges = Object.entries(userBadges || {})
    .map(([key, value]) => {
      ...
      if (equals(key, "predictions")) {
        return renderBadge({
          badge,
          title: `Prediction: ${badgeInfo?.predictions}`,
          value,
        });
      }
      ...
      if (equals(key, "sub-gifter")) {
        return (
          <BadgeWithTooltip
            img={badge.versions[max(index, 0)]?.image_url_1x}
            badge={badge}
            title={"Sub Gifter"}
          />
        );
    ...
    <div className={classes.messageContainer}>
      ...
      <Typography component="span" className={classes.text}>
        <Typography
          ...
          sx={{ color: usernameColor, fontWeight: "bold" }}
        >
          {displayName}
        </Typography>
        :{" "}
